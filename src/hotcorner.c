...
static HotCorner * hotCorner_new(XfcePanelPlugin *plugin) {
    ...
    hotCorner->icon = xfce_panel_image_new_from_source ("org.xfce.settings.display");
...
static GtkWidget * create_layout(HotCorner * hotCorner) {
    ...
    GtkWidget * monitorImage = gtk_image_new_from_icon_name("org.xfce.settings.display", GTK_ICON_SIZE_BUTTON);
...
static void on_open_configure_window(XfcePanelPlugin * plugin, HotCorner * hotCorner) {
    ...
    gtk_window_set_icon_name (GTK_WINDOW (dialog), "org.xfce.settings.display");
